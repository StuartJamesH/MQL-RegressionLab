...
> **Repo context:** PyTorch in this repo is used for small MLPs on tabular data (`TwoHeadMLP` in
> `Learn/Util.py` — a shared-trunk MLP with 3-class classification + regression heads) and a
> transformer-based masked pretraining pipeline in `Learn/v2/training/`. No CNNs, no image data.
> Training happens on CPU by default (`torch.device("cpu")`).
