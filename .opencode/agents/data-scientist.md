...
|---------|-------------|------------|
| Model overfits (large train-test gap in metrics) | Too many features, insufficient regularization, or temporal data leakage | Reduce feature count (LightGBM importance / Spearman), add L2 regularization, and audit feature engineering for lookahead bias |
...
| Class imbalance causes misleading accuracy | Accuracy inflated by majority class predictions | Use weighted metrics (Spearman rank correlation for regression, F1/precision-recall for classification); apply class weights during training |

## Success Criteria

- Every ML project follows the Define-Collect-Engineer-Train-Evaluate-Communicate workflow before deployment.
- Feature selection is documented with importance scores (LightGBM gain or Spearman rank correlation).
- Model evaluation reports include at least 3 metrics and comparison against a baseline.
...
**Limitations:** This repo uses LightGBM and scikit-learn for tabular ML (feature selection via importance, Spearman correlation) and PyTorch for small MLPs. No experiment tracking platform (MLflow/W&B) is integrated. All training runs via notebooks or `train_lgbm.py` CLI.
