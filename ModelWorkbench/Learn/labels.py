...
def _compute_outcomes_nb(highs, lows, atrs, tp_mult, sl_mult):
    ...
    for t0 in range(n - 1):
        ...
        for t1 in range(t0 + 1, n):
            ...
            # Early exit: once both sides are resolved, all remaining excursion
            # tracking is redundant — MFE/MAE for a resolved side is already at
            # its final value and will be capped at 1.0 regardless of future bars.
            # Without this break the loop is O(n²); with it, O(n × avg_hold_bars).
...
def create_quality_targets(
    ...
    """
    # 1. We need the forward rolling max high and min low over `lookahead_bars`
    # We shift(-lookahead_bars) to look forward. To avoid future leakage, the MFE/MAE
    # for index `t` uses High/Low from `t+1` to `t+lookahead_bars`.
    
    # We want max/min for the window [t+1, t+lookahead_bars]
    # We can do this efficiently with pandas rolling on reversed data, or just use 
    # forward-shifted rolling windows. 
    # An easy vectorized way: rolling(window) on High, then shift back.
    # Note: rolling(w).max().shift(-w) gives max of [t+1, t+w]
    ...
    # Max high and min low in the lookahead window (exclusive of current bar)
    # rolling(lookahead_bars).max() at t+lookahead_bars gives max of [t+1, t+lookahead_bars]
    # So we shift(-lookahead_bars) to align it back to t.
    ...
    # Calculate MFE and MAE (absolute price difference)
    ...
    # Fill zero ATR to avoid division by zero
    ...
    # Calculate Quality (log domain to dampen extreme spikes)
...
def MFE_filter_outcomes(df, mfe_thresh=1, mfa_thresh=0.30, label_params=None, outcome_params=None):
    ...
    cols = df.columns # Original columns from input df
...
def generate_tradeability_scores(
    ...
    for fold_number, (train_idx, test_idx) in enumerate(folds, start=1):
        ...
        # print(f"Train Start         : {train_start_ts}")
        # print(f"Train End           : {train_end_ts}")
        # print(f"Test Start          : {test_start_ts}")
        # print(f"Test End            : {test_end_ts}")
        # print(f"Train Size          : {len(train_idx):,}")
        # print(f"Test Size           : {len(test_idx):,}")
        # print(f"Train Positive Rate : {train_pos_rate:.6f}")
        # print(f"Test Positive Rate  : {test_pos_rate:.6f}")
        # print(f"ROC AUC             : {fold_roc_auc:.6f}" if np.isfinite(fold_roc_auc) else "ROC AUC             : nan")
        # print(f"PR AUC              : {fold_pr_auc:.6f}" if np.isfinite(fold_pr_auc) else "PR AUC              : nan")
