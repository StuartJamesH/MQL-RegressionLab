...
class LSTMAttentionSEClassifier(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        se = torch.sigmoid(self.se_expand(F.gelu(self.se_reduce(context))))
        h  = h * se.unsqueeze(1)
...
class TemporalBlock(nn.Module):
    ...
    Conv → GroupNorm → GELU → Dropout order.
    GroupNorm(1, C) replaces BatchNorm (no running statistics, identical
    behaviour in train and eval, robust to distribution shift between periods).
    GELU replaces ReLU (non-zero gradient everywhere, prevents dying neurons).
    """
    ...
    def __init__(
        ...
        # Plain Conv1d — weight_norm removed because Adam can drive a weight_v
        # vector's norm toward zero (g * v/||v|| → NaN) at moderate LRs with
        # deep dilated stacks.  BatchNorm1d now provides per-block activation
        # normalisation (Conv → BN → ReLU → Dropout order).
        self.conv1 = nn.Conv1d(n_inputs, n_outputs, kernel_size, dilation=dilation, padding=0)
        self.conv2 = nn.Conv1d(n_outputs, n_outputs, kernel_size, dilation=dilation, padding=0)

        # GroupNorm(1, C) replaces BatchNorm1d:
        #   - BatchNorm uses running statistics accumulated from training batches.
        #     When the train and val periods have different feature distributions
        #     (common for time-series), the running stats are wrong at inference
        #     time → BN in eval mode produces a completely different activation
        #     scale from training, causing all outputs to collapse to the same
        #     constant (NaN Spearman in validation despite declining train loss).
        #   - GroupNorm has NO running statistics.  It normalises each sample
        #     independently (over all C channels per timestep), so train and eval
        #     mode are identical and distribution shift is handled automatically.
        self.gn1 = nn.GroupNorm(1, n_outputs)
        self.gn2 = nn.GroupNorm(1, n_outputs)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = self.dropout1(self.act1(self.gn1(self.conv1(pad_x))))

        pad_out = F.pad(out, (self.padding, 0))
        out = self.dropout2(self.act2(self.gn2(self.conv2(pad_out))))
...
class TCNDualHeadRegressor(nn.Module):
    ...
    def __init__(
        ...
        # Shared projection — GELU gives smoother gradients than ReLU for regression
        ...
        # • No intra-head Dropout: shared_fc already applies one dropout; stacking
        #   three Dropout layers compounds to (1-p)³ effective rate, killing gradients.
        # • Softplus replaces final ReLU: if pre-activation ≤ 0, ReLU gradient = 0 and
        #   the output saturates at 0 for the rest of training (dead-output problem).
        #   Softplus = log(1+exp(x)) is smooth, always positive, always differentiable.
        ...
        # Short Regression Head (same rationale)
...
class HybridTCNTabularSignedRegressor(nn.Module):
    """
    Hybrid model that fuses temporal TCN features with last-bar tabular features.

    Outputs three heads:
      - long_quality (non-negative)
      - short_quality (non-negative)
      - signed_quality (real-valued, rank-optimised)
    """
    ...
    def __init__(
        ...
        signed_head_width_mult: float = 1.0,
        signed_head_depth: int = 2,
        use_learned_query: bool = True,
        se_context_window: int = 32,
    ):
        super().__init__()
        ...
        self.tabular_proj = nn.Sequential(
            nn.Linear(input_dim, tabular_hidden),
            nn.GELU(),
            nn.Dropout(dropout_out),
        ...
        fusion_in = hidden_channels + tabular_hidden
        ...
        signed_hidden = max(16, int((fusion_hidden // 2) * signed_head_width_mult))
        signed_head_depth = max(1, int(signed_head_depth))
        signed_layers = []
        in_dim = fusion_hidden
        for _ in range(signed_head_depth):
            signed_layers.append(nn.Linear(in_dim, signed_hidden))
            signed_layers.append(nn.GELU())
            in_dim = signed_hidden
        signed_layers.append(nn.Linear(in_dim, 1))
        self.signed_head = nn.Sequential(*signed_layers)

    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        # x: (B, S, F)
        bsz = x.size(0)

        h = self.tcn(x.transpose(1, 2)).transpose(1, 2).contiguous()
        h = self.ln(h)
        ...
        query = self.query_token.expand(bsz, -1, -1) if self.use_learned_query else context.unsqueeze(1)
        pooled, _ = self.attn(query, h, h, need_weights=False)
        temporal_feat = self.temporal_proj(pooled.squeeze(1))

        last_bar = x[:, -1, :]
        tab_feat = self.tabular_proj(last_bar)

        fused = self.fusion(torch.cat([temporal_feat, tab_feat], dim=1))
        long_quality = self.long_head(fused).squeeze(-1)
        short_quality = self.short_head(fused).squeeze(-1)
        signed_quality = self.signed_head(fused).squeeze(-1)
